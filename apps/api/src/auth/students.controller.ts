...
export class StudentsController {
  ...
  async listStudents(
    ...
    const studentNumber = /^\d+$/.test(normalizedSearch)
      ...
      : undefined;
    const phoneHmac = normalizedSearch.startsWith('01')
      ? this.securityService.generatePhoneHmac(normalizedSearch)
      : undefined;
    const parentPhoneHmac = normalizedSearch.startsWith('01')
      ? this.securityService.generatePhoneHmac(normalizedSearch)
      : undefined;
    const emailHmac = normalizedSearch.includes('@')
    ...
    const where = {
      ...
      ...(normalizedSearch && !phoneHmac && !emailHmac
        ? {
            OR: [
