...
describe('Academy Web Home', () => {
  ...
  it('renders the social platform links in the footer', async () => {
    render(await Home());

    const facebook = screen.getByRole('link', { name: /فيسبوك/ });
    const youtube = screen.getByRole('link', { name: /يوتيوب/ });
    const tiktok = screen.getByRole('link', { name: /تيك توك/ });
    expect(facebook).toHaveAttribute('target', '_blank');
    expect(facebook).toHaveAttribute('rel', 'noopener noreferrer');
    expect(facebook).toHaveAttribute(
      'href',
      'https://www.facebook.com/share/1BT3CAL2iT/',
    );
    expect(youtube).toHaveAttribute('href', 'https://youtube.com/@el_bahrawy1?si=4BQ_nvvW7iN_UXN2');
    expect(tiktok).toHaveAttribute('href', 'https://www.tiktok.com/@el_bahrawy_3?_r=1&_t=ZS-98Zhg400QwW');
  });
});
