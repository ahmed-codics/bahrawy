            ...
            <div className="mt-5 grid gap-3">
              {normalizeOptions(entry.question.options).map((option) => {
                ...
                return (
                  <label
                    key={option.id}
                    className={`flex min-h-14 cursor-pointer items-center gap-3 rounded-xl border p-4 transition ${selected ? 'border-brand-500 bg-brand-50 text-brand-700 dark:bg-brand-950/30 dark:text-brand-200' : 'border-border-default hover:border-brand-300 hover:bg-surface-soft'}`}
                  >
                    <input
                      type="radio"
                ...
                {questions.map((entry, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`ba-number flex size-10 items-center justify-center rounded-xl font-bold transition hover:scale-105 ${isAnswered ? 'bg-brand-600 text-white shadow-md' : 'bg-surface-soft text-text-muted hover:bg-surface-hover hover:text-text'}`}
