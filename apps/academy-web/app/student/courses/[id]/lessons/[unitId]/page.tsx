...
type UnitDetail = {
  unit: {
    id: string;
    titleAr: string;
    chapter: { titleAr: string; course: { id: string; titleAr: string } };
...
export default function LessonPage({
  ...
  if (!detail)
    return (
      <EmptyState
        ...
        onAction={() => router.push(`/student/courses/${id}`)}
      />
    );
  ...
  const openItem = (item: ContentItem) => {
    if (item.type === 'ASSESSMENT' && item.assessmentId)
      router.push(`/student/assessments/${item.assessmentId}`);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-7 pb-10">
      <button
        ...
        className="ba-focus inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-sm font-black text-text-muted hover:bg-surface-soft hover:text-ink"
      >
        <ArrowRight className="size-4" /> العودة إلى دروس الكورس
      </button>

      <section className="student-hero student-entrance overflow-hidden">
        <div className="grid min-h-0 lg:min-h-[22rem] lg:grid-cols-[minmax(0,1fr)_22rem]">
          <div className="flex flex-col justify-center px-6 py-9 sm:px-10 lg:px-12">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              <Badge tone={hasAccess ? 'success' : 'neutral'}>
                {hasAccess ? (
                  <>
                    <CheckCircle2 className="size-4" /> الدرس مفتوح
                  </>
                ) : (
                  <>
                    <LockKeyhole className="size-4" /> يحتاج شراء
                  </>
                )}
              </Badge>
          ...
          <div className="student-cover relative aspect-video min-h-0 border-t border-border-default lg:aspect-auto lg:min-h-60 lg:border-r lg:border-t-0">
            {lessonProduct?.coverImageUrl ? (
              ...
              <div className="flex h-full items-center justify-center">
                <PlayCircle className="size-24 text-cyan-200/40" />
              </div>
            )}
          </div>
        </div>
          ...
          <div>
            ...
            {detail.access?.reason === 'PREREQUISITE' && (
              <p className="mt-3 text-sm font-bold text-warning">
                أكمل {detail.access.prerequisite?.titleAr || unit.prerequisiteAssessment?.titleAr}{' '}
                أولاً.
              </p>
            )}
          </div>
      ...
      <section>
        <div className="mb-5">
          <p className="text-sm font-black text-brand-700 dark:text-brand-300">داخل الدرس</p>
          <h2 className="ba-heading mt-1 text-3xl">محتوى {unit.titleAr}</h2>
        </div>
        {!contentItems.length ? (
          <EmptyState
            title="لا يوجد محتوى منشور بعد"
            description="سيظهر الفيديو والملفات هنا بعد نشرها."
          />
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {contentItems.map((item, index) => (
              <ContentCard
                key={item.lessonId || item.assessmentId || `${item.type}-${index}`}
                item={item}
                index={index + 1}
                hasAccess={hasAccess}
      ...
      <div className="flex gap-3 rounded-2xl border border-brand-200/60 bg-brand-50/60 p-4 text-sm text-brand-900 dark:border-brand-900 dark:bg-brand-950/20 dark:text-brand-100">
        <ShieldCheck className="size-5 shrink-0" />
        <p className="font-bold">
          المحتوى محمي بحسابك، والوصول لا ينتقل إلى أي درس آخر إلا بشرائه أو امتلاك الكورس أو
          الباقة.
        </p>
      </div>
    </div>
...
function ContentCard({
  ...
  const isActiveAssessment =
    item.type === 'ASSESSMENT' && item.attempt && !item.attempt.submittedAt;
  ...
  return (
    <article className={`student-course-card relative p-5 ${!allowed ? 'opacity-90' : ''}`}>
      ...
      <div className="flex items-start gap-4">
        <span className="ba-number flex size-10 shrink-0 items-center justify-center rounded-xl bg-surface-soft text-sm font-black">
          {index}
        </span>
        <span
          className={`flex size-11 shrink-0 items-center justify-center rounded-xl ${isActiveAssessment ? 'bg-warning/20 text-warning-700 dark:bg-warning/10 dark:text-warning-300' : 'bg-brand-100 text-brand-700 dark:bg-brand-950/40 dark:text-brand-200'}`}
        >
        ...
        <div className="min-w-0 flex-1">
          ...
          {item.durationSeconds ? (
            <p className="mt-1 text-xs text-text-muted">
              {Math.ceil(item.durationSeconds / 60)} دقيقة
            </p>
          ) : item.questionCount ? (
